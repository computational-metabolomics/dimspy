...
if __name__ == "__main__":
    ...
    # We probably need to move this to a separate python file
    import argparse
    import subprocess
    from collections import OrderedDict
    import cPickle as pickle

    parser = argparse.ArgumentParser(description='This is a python implemention of MSFilReader by Ralf Weber.')
    parser.add_argument('-i', '--input', help='Input RAW file', required=True)
    parser.add_argument('-o', '--output', help='Output pkl file', required=True)
    parser.add_argument('-m', '--mode', help='Output pkl file')
    args = parser.parse_args()
    print (args)

    hraw = ThermoRawfile(args.input)

    scan_ids, spectra, noise, extra, tic = OrderedDict(), OrderedDict(), OrderedDict(), OrderedDict(), OrderedDict()
    for scan_id in range(hraw.GetFirstSpectrumNumber(), hraw.GetLastSpectrumNumber() + 1):

        header = hraw.GetFilterForScanNum(scan_id)
        scan_ids.setdefault(header, []).append(scan_id)
        if args.mode.lower() == "centroid":
            peaklist = hraw.GetMassListFromScanNum(scan_id, centroidResult=True)  # bCentroidResult,
        else:
            peaklist = hraw.GetMassListFromScanNum(scan_id)
        spectra[scan_id] = [peaklist[0][0], peaklist[0][1]]

        extra[scan_id] = type(hraw.GetTrailerExtraForScanNum(scan_id))
        noise[scan_id] = type(hraw.GetNoiseData(scan_id))
        tic[scan_id] = hraw.GetScanHeaderInfoForScanNum(scan_id)["TIC"]
    # We might need a few more parameters.
    d = {"SanNums":scan_ids, "MassLists":spectra, "InstName":str(hraw.GetInstName()), "noise":noise, "extra":extra, "TIC":tic}
    ...
    try:
        ...
        p = subprocess.Popen(["taskkill", "/pid", current_pid, "/f"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
