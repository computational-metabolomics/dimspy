...
def set_logging(level = logging.INFO, print_formatter = None, log_file = None, log_mode = 'w', log_formatter = None, exception_format = None):
    logger = logging.getLogger()
    logger.handlers = [] # remove existing handlers
    logger.setLevel(level)

    # log to terminal
    assert print_formatter is None or isinstance(print_formatter, logging.Formatter), 'print formatter is not a Formatter object'
    ...
    if log_file is not None:
        assert log_formatter is None or isinstance(log_formatter, logging.Formatter), 'log file formatter is not a Formatter object'
        lfmt = logging.Formatter('%(asctime)-15s : [%(levelname)s] >> %(message)s') if log_formatter is None else log_formatter
