...
class PeakList_Tags(object):
    ...
    def has_tag(self, *args, **kwargs):
        if len(args) + len(kwargs) > 1: raise ValueError('searching multiple tags is not allowded')
        return (args[0] in self.tag_values) if len(args) > 0 else (kwargs.items()[0] in self.typed_tags)

    def tag_of(self, tag_type = None):
        if not (tag_type is None or self.has_tag_type(tag_type)): raise KeyError('unknown tag type [%s]' % tag_type)
